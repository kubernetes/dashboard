...
### Allow accessing dashboard from outside the container (2.7 or less)
...
### Change port number for dashboard (2.7 or less)
...
### To run dashboard using Docker at ease (2.7 or less)
...
### To run npm commands as you want (**2.7 or less**)
...
5. Run `hack/develop/run-npm-on-container.sh`.

These manipulations will build container and run dashboard with `make run` as default.

### Just to run development container without creating cluster and running dashboard

1. Set `K8S_DASHBOARD_CMD` environment variable as `bash`.
2. Run `hack/develop/run-npm-on-container.sh`.
