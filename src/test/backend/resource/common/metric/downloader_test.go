...
type PodData map[string][]RawDataPoint
type NodeData map[string][]RawDataPoint
...
func (self FakeRequest) DoRaw() ([]byte, error) {
	...
	if strings.Contains(path, "/pod-list/") {
		...
		items := []HeapsterJSONFormat{}
		for _, pod := range requestedPods {
			items = append(items, HeapsterJSONFormat{self.PodData[pod+"/"+namespace]})
		}
		x, err := json.Marshal(HeapsterJSONAllInOneFormat{items})
		...
		x, err :=  json.Marshal(HeapsterJSONFormat{self.NodeData[requestedNode]})
		log.Println("Got you:",string(x))
		return x, err
	} else {
		return nil, fmt.Errorf("Invalid request url %s", path)
...
func NewRawDPs(dps []int64, startTime int) []RawDataPoint {
	newRdps := []RawDataPoint{}
	for i:=0;i<len(dps)&&startTime+i<10;i++ {
		newRdps = append(newRdps, RawDataPoint{fmt.Sprintf(TIME_TEMPLATE, i+startTime), dps[i]})
	}
	return newRdps
}
...
func fakeHeapsterSelector(resourceType MetricResourceType, namespace string, resourceNames []string) HeapsterSelector {
	...
	testCases := []HeapsterSelectorTestCase{
		{
			...
			fakeHeapsterSelector(ResourceTypePod, "a", []string{"P1"}),
			NewDps([]int64{0,5,10},0),
			1,
		},
		{
			"get data for 3 pods",
			fakeHeapsterSelector(ResourceTypePod, "a", []string{"P1", "P2", "P3"}),
			NewDps([]int64{45,60,75},0),
			1,
		},
		{
			"get data for 4 pods where 1 pod does not exist - ignore non existing pod",
			fakeHeapsterSelector(ResourceTypePod, "a", []string{"P1", "P2", "P3", "NON_EXISTING"}),
			NewDps([]int64{45,60,75},0),
			1,
		},
		{
			"get data for 4 pods where pods have different X timestams available",
			fakeHeapsterSelector(ResourceTypePod, "b", []string{"P1", "P2", "P3", "P4"}),
			NewDps([]int64{1000,2300,2700,1500},0),
			1,
		},
		{
			"ask for non existing namespace - return no data points",
			fakeHeapsterSelector(ResourceTypePod, "NON_EXISTING_NAMESPACE", []string{"P1"}),
			NewDps([]int64{},0),
			1,
		},
		{
			"get data for 0 pods - return no data points",
			fakeHeapsterSelector(ResourceTypePod, "b", []string{}),
			NewDps([]int64{},0),
			0,
		},
		{
			"get data for 0 nodes - return no data points",
			fakeHeapsterSelector(ResourceTypeNode, "NO_NAMESPACE", []string{}),
			NewDps([]int64{},0),
			0,
		},
		{
			"ask for 1 node",
			fakeHeapsterSelector(ResourceTypeNode, "NO_NAMESPACE", []string{"N1"}),
			NewDps([]int64{0,5,10},0),
			1,
		},
		{
			"ask for 3 nodes",
			fakeHeapsterSelector(ResourceTypeNode, "NO_NAMESPACE", []string{"N1", "N2", "N3"}),
			NewDps([]int64{45,60,75},0),
...
var selectorPoll = HeapsterSelectors{
	fakeHeapsterSelector(ResourceTypePod, "a", []string{"P1"}),
	fakeHeapsterSelector(ResourceTypePod, "a", []string{"P2", "P3", "P4"}),
	fakeHeapsterSelector(ResourceTypePod, "a", []string{"P3", "P4"}),
	fakeHeapsterSelector(ResourceTypePod, "b", []string{"P1", "P2", "P3"}),
	fakeHeapsterSelector(ResourceTypePod, "b", []string{"P2", "P3", "P4"}),
	fakeHeapsterSelector(ResourceTypeNode, "NO_NAMESPACE", []string{"N1", "N2", "N3"}),
	fakeHeapsterSelector(ResourceTypeNode, "NO_NAMESPACE", []string{"N3", "N4"}),
}
