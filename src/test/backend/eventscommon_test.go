...
func TestFilterEventsByPodsName(t *testing.T) {
	cases := []struct {
		events   []api.Event
		pods     []api.Pod
		expected []api.Event
		...
		{
			[]api.Event{
				{InvolvedObject: api.ObjectReference{Name: "TestPod"}},
				{InvolvedObject: api.ObjectReference{Name: "TestObject"}},
			},
			[]api.Pod{
	...
	for _, c := range cases {
		actual := filterEventsByPodsName(c.events, c.pods)
		if !reflect.DeepEqual(actual, c.expected) {
