...
import BreadcrumbsController from 'common/components/breadcrumbs/breadcrumbs_controller';
...
describe('Breadcrumbs controller ', () => {
  ...
  beforeEach(() => {
    ...
    angular.mock.inject(($controller, $state, $interpolate) => {
      state = $state;
  ...
  it('should call init breadcrumbs', () => {
    // given
    spyOn(BreadcrumbsController.prototype, 'initBreadcrumbs');

    // when
    let mockCtrl = new BreadcrumbsController();

    // then
    expect(mockCtrl.initBreadcrumbs).toHaveBeenCalled();
  });
