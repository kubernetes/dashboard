...
import {LabelKeyNameLengthType} from 'common/validators/types/labelkeynamelengthtype';

describe('Label key name length type', () => {
  /** @type {!LabelValuePatternType} */
  let labelKeyNameLengthType;

  beforeEach(() => {
    angular.mock.inject(() => { labelKeyNameLengthType = new LabelKeyNameLengthType(); });
  });
  ...
  it('should set validity to false when key name exceeds 63 characters ', () => {
    ...
    failKeyNames.forEach(
        (failKeyName) => { expect(labelKeyNameLengthType.isValid(failKeyName)).toBeFalsy(); });
  });
  ...
  it('should set validity to true when key name does not exceed 63 characters ', () => {
    ...
    // then
    passKeyNames.forEach(
        (passKeyName) => { expect(labelKeyNameLengthType.isValid(passKeyName)).toBeTruthy(); });
  });
});
