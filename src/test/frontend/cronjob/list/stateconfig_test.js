...
describe('StateConfig for cron job list', () => {
  ...
  it('should resolve cron jobs', angular.mock.inject(($q) => {
    ...
    let actual = resolveCronJobList(resource, {namespace: 'foo'}, kdDataSelectService);

    expect(resource.get).toHaveBeenCalledWith(kdDataSelectService.getDefaultResourceQuery('foo'));
    expect(actual).toBe(promise);
  }));

  it('should resolve cron jobs with no namespace', angular.mock.inject(($q) => {
    let promise = $q.defer().promise;
    ...
    let actual = resolveCronJobList(resource, {}, kdDataSelectService);
