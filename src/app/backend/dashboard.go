...
var (
	...
	argAuthenticationModes = pflag.StringSlice("authentication-modes", []string{string(authApi.Token)}, "Enables authentication options that will be reflected on login screen. Supported values: token, basic. Default: token")
)
...
func initAuthManager(clientManager client.ClientManager, tokenTTL time.Duration) authApi.AuthManager {
	...
	return auth.NewAuthManager(clientManager, tokenManager, authApi.ToAuthenticationModes(*argAuthenticationModes))
}
