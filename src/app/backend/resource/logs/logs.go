...
// Returns all logs.
var NoLogViewSelector = &LogViewSelector{
...
// For example line in the middle of array is not likely to be deleted in the near future and therefore it serves as a good reference point.
...
func (self LogLines) GetLineIndex(logLineId *LogLineId) int {
	if logLineId == nil || logLineId.LogTimestamp == NewestTimestamp || len(self) == 0 {
...
func (self LogLines) SelectLogs(logSelector *LogViewSelector) (LogLines, LogLineId, LogLineId, LogViewInfo) {
	...
	if referenceLineIndex == -1 || requestedNumItems <= 0 || len(self) == 0 {
		// Requested reference line could not be found, probably it's already gone or requested no logs. Return no logs.
		return LogLines{}, logSelector.ReferenceLogLineId, logSelector.ReferenceLogLineId, LogViewInfo{}
	}
