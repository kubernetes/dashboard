...
func GetNodeDetail(client k8sClient.Interface, heapsterClient client.HeapsterClient, name string) (
	*NodeDetail, error) {
	...
	events, err := event.GetNodeEvents(client, *node)
	if err != nil {
		return nil, err
	}
...
func getNodePods(client k8sClient.Interface, heapsterClient client.HeapsterClient,
	...
	fieldSelector, err := fields.ParseSelector("spec.nodeName=" + node.ObjectMeta.Name +
...
func toNodeDetail(node api.Node, pods pod.PodList, events []api.Event) NodeDetail {
