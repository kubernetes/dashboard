...
type ResourceSelector struct {
	...
	ResourceType  metric.MetricResourceType
...
func (self *ResourceSelector) GetHeapsterSelector(cachedPods []api.Pod) (metric.HeapsterSelector, error) {
	summingResource, isDerivedResource := metric.DerivedResources[self.ResourceType]
	if !isDerivedResource {
		return metric.NewHeapsterSelectorFromNativeResource(self.ResourceType, self.Namespace, []string{self.ResourceName})
	}
	...
	if summingResource == metric.ResourceTypePod {
		myPods, err := self.getMyPodsFromCache(cachedPods)
		if err != nil {
			return metric.HeapsterSelector{}, err
		}
		return metric.NewHeapsterSelectorFromNativeResource(metric.ResourceTypePod, self.Namespace,  podListToNameList(myPods))
	} else {
		// currently can only convert derived resource to pods. You can change it by implementing other methods
		return metric.HeapsterSelector{}, fmt.Errorf(`Internal Error: Requested summing resource is not supported. Requested "%s"`, summingResource)
	}
}
