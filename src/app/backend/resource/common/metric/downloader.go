...
func (self HeapsterSelectors) DownloadMetric(client client.HeapsterClient, metricName string) MetricPromises {
	...
	for _, compressedSelector := range compressedSelectors {
		go func(selector HeapsterSelector) {
			unassignedResourcePromises := selector.downloadMetricForEachTargetResource(client, metricName)
...
func removeDuplicates(list []string) []string {
	dummyMap := map[string]bool{}
	for _, e := range list {
		dummyMap[e] = false
	}
	uniqueList := make([]string, len(dummyMap))
	i := 0
	for e := range dummyMap {
		uniqueList[i] = e
		i++
	}
	return uniqueList
...
func (self HeapsterSelectors) compress() (HeapsterSelectors, map[string][]int) {
	...
	resourceTypeMap := map[string]MetricResourceType{}
...
func NewHeapsterSelectorFromNativeResource(resourceType MetricResourceType, namespace string, resourceNames []string) (HeapsterSelector, error) {
	...
	if resourceType == ResourceTypePod {
		return HeapsterSelector{
			TargetResourceType: ResourceTypePod,
		...
		return HeapsterSelector{
			TargetResourceType: ResourceTypeNode,
			Path: `nodes/`,
			Resources: resourceNames,
			Label: Label{resourceType:resourceNames},
		}, nil
...
type HeapsterSelector struct {
	TargetResourceType  MetricResourceType
	Path     string
	Resources []string
	Label
}
