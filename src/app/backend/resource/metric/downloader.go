...
func (self HeapsterSelectors) DownloadMetric(client client.HeapsterClient, metricName string) MetricPromises {
	...
	unassignedResourcePromisesMap := map[int]MetricPromises{}
	for selectorId, compressedSelector := range compressedSelectors {
		unassignedResourcePromisesMap[selectorId] = compressedSelector.downloadMetricForEachTargetResource(client, metricName)
	...
	go func() {
		...
		for selectorId, selector := range compressedSelectors {
			unassignedResourcePromises := unassignedResourcePromisesMap[selectorId]
