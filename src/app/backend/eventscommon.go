...
func GetPodsEventWarnings(eventList *api.EventList, pods []api.Pod) []Event {
	...
	// Filter events by failed pods name
	events = filterEventsByPodsName(events, failedPods)
...
func filterEventsByPodsName(events []api.Event, pods []api.Pod) []api.Event {
	result := make([]api.Event, 0)
	podEventMap := make(map[string]bool, 0)

	for _, pod := range pods {
		podEventMap[pod.Name] = true
	}

	for _, event := range events {
		if _, exists := podEventMap[event.InvolvedObject.Name]; exists {
			result = append(result, event)
		}
	}
