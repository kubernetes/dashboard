...
export default class CreateSecretController {
  ...
  constructor($mdDialog, $log, $resource, errorDialog, namespace) {
     ...
     */
    this.messages = createSecretMessages;
  }
...
const createSecretMessages = {
  /** @export @type string @desc Create image pull secret dialog title. The message appears at the top of the dialog box. */
  MSG_IMAGE_PULL_SECRET_CREATE_DIALOG_TITLE: goog.getMsg('Create a new image pull secret'),

  /** @export @type string @desc Create image pull secret dialog subtitle. Appears right below the title. */
  MSG_IMAGE_PULL_SECRET_CREATE_DIALOG_SUBTITLE:
      goog.getMsg('The new secret will be added to the cluster.'),

  /** @export @type string @desc Label 'Secret name', which appears as a placeholder in an empty input field in the image pull secret creation dialog. */
  MSG_IMAGE_PULL_SECRET_NAME_LABEL: goog.getMsg('Secret name'),

  /** @export @type string @desc The text appears when the image pull secret name does not match the expected pattern. */
  ...
  /** @export @type string @desc The text appears when the image pull secret name exceeds the maximal length. */
  ...
  MSG_IMAGE_PULL_SECRET_NAME_REQUIRED: goog.getMsg('Name is required.'),

  /** @export @type string @desc  User help text for the create image pull secret dialog. Directly after this text a specific namespace is specified. */
  ...
  /** @export @type string @desc Label 'Image pull secret data', which appears as a placeholder in an empty input field in the image pull secret creation dialog. */
  MSG_IMAGE_PULL_SECRET_DATA_LABEL: goog.getMsg('Image pull secret data'),

  /** @export @type string @desc Warning which tells the user that the image pull secret data is required. */
  MSG_IMAGE_PULL_SECRET_DATA_REQUIRED: goog.getMsg('Data is required'),

  /** @export @type string @desc The text appears when the image pull secret data is not Base64 encoded. */
  MSG_IMAGE_PULL_SECRET_DATA_BASE64_WARNING: goog.getMsg('Data must be Base64 encoded'),

  /** @export @type string @desc User help text for the input of the "Image Pull Secret" data. */
