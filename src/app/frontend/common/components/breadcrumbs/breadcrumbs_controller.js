...
export default class BreadcrumbsController {
  ...
  constructor($state, $interpolate) {
    ...
    /**
     * @private {number} - it is defined to allow middle ellipsis formatting on long breadcrumb
     * labels. Label space will be equally divided between all breadcrumbs. Unit: pixels.
     */
    this.maxBreadcrumbsLength_ = 300;
  }
  ...
  canAddBreadcrumb(breadcrumbs) {
    return this.limit === undefined ||
        (this.limit !== undefined && this.limit > breadcrumbs.length);
  }
  ...
  /**
   * Returns style for the specific breadcrumb. Width per breadcrumb has to be calculated in
   * order to allow middle ellipsis formatting and container has to have width specified.
   *
   * @return {{width: string, display: string}}
   * @export
   */
  getStyle() {
    return {
      'width': `${this.maxBreadcrumbsLength_ / this.breadcrumbs.length}px`,
