 ...
 * @param {!angular.Attributes} attributes - directive attributes (have to contain kdMax)
 * @return {boolean}
 */
export function shouldSetWarning(inputCtrl, attributes) {
  /** @type {number} */
  let number = Number.parseInt(inputCtrl.$viewValue, 10);

  return !hasWarning(inputCtrl) && number > attributes[max];
}
...
/**
 * Returns true if kdMax attribute is set to true on input controller, false otherwise.
 *
 * @param {!angular.NgModelController} inputCtrl
 * @return {boolean}
 */
