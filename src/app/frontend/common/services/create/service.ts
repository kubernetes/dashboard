...
export class CreateService {
  ...
  async createContent(
    ...
    try {
      const {token} =
          await this.csrfToken_.getTokenForAction('appdeploymentfromfile')
              .toPromise();
      ...
      if (response.error.length > 0) {
        this.reportError(
            i18n.MSG_DEPLOY_DIALOG_PARTIAL_COMPLETED, response.error);
      }
    } catch (err) {
      error = err;
    }
  ...
  async deploy(spec: AppDeploymentSpec): Promise<AppDeploymentContentResponse> {
    ...
    try {
      const {token} =
          await this.csrfToken_.getTokenForAction('appdeployment').toPromise();
      this.isDeployInProgress_ = true;
      response = await this.http_
                     .post<AppDeploymentContentResponse>(
                         'api/v1/appdeployment', spec, {
                           headers: {[this.CONFIG.csrfHeaderName]: token},
                         })
                     .toPromise();
    } catch (err) {
