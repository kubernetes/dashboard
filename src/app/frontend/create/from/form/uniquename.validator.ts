...
@Directive({
  ...
  providers: [{ provide: NG_ASYNC_VALIDATORS,  useExisting: forwardRef(() => UniqueNameValidator), multi: true }]
})
export class UniqueNameValidator implements AsyncValidator, Validator, OnChanges {
  ...
  validate(control: AbstractControl): Observable<{ [key: string]: boolean }> {
    if (!control.value) {
      ...
      }).pipe(debounceTime(500), map(res => !res.valid ? { [uniqueNameValidationKey]: control.value } : null));
    }
  }
