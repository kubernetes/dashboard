...
export class CreateFromFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.form = this.fb_.group({
      ...
      description: [''],
      namespace:
          [this.route_.snapshot.params.namespace || '', Validators.required],
      imagePullSecret: [''],
      cpuRequirement: [
        '',
    ...
    this.labelArr =
        [new DeployLabel(APP_LABEL_KEY, '', false), new DeployLabel()];
  ...
  getSecrets(): void {
    this.http_.get(`api/v1/secret/${this.namespace.value}`)
  ...
  /**
   * Returns true if more options have been enabled and should be shown, false
   * otherwise.
   */
  ...
  handleNamespaceDialog(): void {
    ...
    dialogDef.afterClosed().take(1).subscribe(answer => {
      /**
       * Handles namespace dialog result. If namespace was created successfully
       * then it will be selected, otherwise first namespace will be selected.
       */
      if (answer) {
  ...
  handleCreateSecretDialog(): void {
    ...
    dialogDef.afterClosed().take(1).subscribe(response => {
      /**
       * Handles create secret dialog result. If the secret was created
       * successfully, then it will be selected, otherwise None is selected.
       */
      if (response) {
  ...
  deploy(): void {
    ...
    const spec: AppDeploymentSpec = {
      ...
      imagePullSecret: this.imagePullSecret.value ? this.imagePullSecret.value :
                                                    null,
      containerCommand:
          this.containerCommand.value ? this.containerCommand.value : null,
