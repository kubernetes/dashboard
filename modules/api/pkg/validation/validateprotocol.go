...
func ValidateProtocol(spec *ProtocolValiditySpec) *ProtocolValidity {
	klog.V(1).Infof("Validating %s protocol for service with external set to %v", spec.Protocol, spec.IsExternal)
	...
	klog.V(1).Infof("Validation result for %s protocol is %v", spec.Protocol, isValid)
