...
func TestDeleteNowShouldPropagateErrorsAndChooseClient(t *testing.T) {
	verber := resourceVerber{
		client:     &FakeRESTClient{err: errors.NewInvalid("err")},
		appsClient: &FakeRESTClient{err: errors.NewInvalid("err from apps")},
	}

	err := verber.Delete("replicaset", true, "bar", "baz", true)

	if !reflect.DeepEqual(normalize(err.Error()), "Delete /api/v1/namespaces/bar/replicasets/baz: err from apps") {
		t.Fatalf("Expected error on verber delete but got %#v", err.Error())
	}

	err = verber.Delete("service", true, "bar", "baz", true)

	if !reflect.DeepEqual(normalize(err.Error()), "Delete /api/v1/namespaces/bar/services/baz: err") {
		t.Fatalf("Expected error on verber delete but got %#v", err.Error())
	}

	err = verber.Delete("statefulset", true, "bar", "baz", true)
...
func TestDeleteNowShouldThrowErrorOnUnknownResourceKind(t *testing.T) {
	verber := resourceVerber{
		client:              &FakeRESTClient{},
		apiExtensionsClient: &FakeRESTClient{err: errors.NewNotFound("err")},
	}
...
func TestDeleteNowShouldRespectNamespacednessOfResourceKind(t *testing.T) {
	verber := resourceVerber{client: &FakeRESTClient{}}

	err := verber.Delete("service", false, "", "baz", true)
...
func TestDeleteNowShouldRespectNotNamespacednessOfResourceKind(t *testing.T) {
	verber := resourceVerber{client: &FakeRESTClient{}}

	err := verber.Delete("namespace", true, "bar", "baz", true)
