 ...
 */
gulp.task('extract-translations', ['scripts'], function(doneFn) {
  let next = null;
  conf.translations.forEach((translation) => {
    if (next == null) {
      next = extractForLanguage(translation.key);
    } else {
      next = next.then(extractForLanguage(translation.key));
    }
  });
  next.then(doneFn).fail((error) => doneFn(error));
});
