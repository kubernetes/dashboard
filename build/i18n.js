...
// Regex to match [[Foo | Bar]] i18n placeholders.
const I18N_REGEX = /\[\[([^|]*?)(?:\|(.*?))?\]\]/mg;
...
export function processI18nMessages(file, minifiedHtml) {
  ...
  function findI18nMessages(htmlContent) {
    ...
    if (matches) {
      return matches.map((match) => {
        ...
        return {text: exec[1], desc: exec[2].trim(), original: match};
      });
    }
    return [];
  }
   ...
   * @return {string}
   */
  function createMessageVarName(index) { return `MSG_${messageVarPrefix}_${index}`; }
